...
class RefreshCompetitionRankings implements Job {
  ...
  async handle(): Promise<void> {
    const allCompetitions = await competitionService.getList({}, { limit: 100_000, offset: 0 });

    await Promise.all(
      allCompetitions.map(async competition => {
        const currentScore = competition.score;
        const newScore = await this.calculateScore(competition);
  ...
  async calculateScore(competition: Competition): Promise<number> {
    const now = new Date();
    let score = 0;
