...
const schema = z
  .object({
    oldName: z.string().min(1).max(12),
    newName: z.string().min(1).max(12)
  })
...
type SubmitNameChangeParams = z.infer<typeof schema>;

class SubmitNameChangeService {
  validate(payload: any): SubmitNameChangeParams {
    return schema.parse(payload);
  }

  async execute(params: SubmitNameChangeParams): Promise<NameChange> {
...
export default new SubmitNameChangeService();
