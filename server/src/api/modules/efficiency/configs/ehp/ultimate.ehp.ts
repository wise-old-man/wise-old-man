import { Skill } from '../../../../../utils';

export default [
  {
    skill: Skill.ATTACK,
    methods: [
      {
        startExp: 0,
        rate: 207_800,
        description: 'Chally TzHaar with 2 alts'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.DEFENCE,
    methods: [
      {
        startExp: 0,
        rate: 539_100,
        description: 'Defensive chinning (286k defence rate, incl. passive XP)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.STRENGTH,
    methods: [
      {
        startExp: 0,
        rate: 300_000,
        description: 'Chally TzHaar with 2 alts'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.RANGED,
    methods: [
      {
        startExp: 0,
        rate: 21_000,
        description: 'Quests, Pest Control for void'
      },
      {
        startExp: 273_742,
        rate: 533_000,
        description: 'Chinning (478k ranged rate, incl. prayer XP)'
      },
      {
        startExp: 737_627,
        rate: 628_000,
        description: 'Chinning (551k ranged rate, incl. prayer XP)'
      },
      {
        startExp: 1_986_068,
        rate: 716_000,
        description: 'Chinning (617k ranged rate, incl. prayer XP)'
      },
      {
        startExp: 5_346_332,
        rate: 801_000,
        description: 'Chinning (678k ranged rate, incl. prayer XP)'
      },
      {
        startExp: 13_034_431,
        rate: 1_085_000,
        description: 'Chinning (776k ranged rate, incl. prayer XP)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.PRAYER,
    methods: [
      {
        startExp: 0,
        rate: 30_000,
        description: 'Quests, blue dragons'
      },
      {
        startExp: 50_339,
        rate: 50_000,
        description: 'Wilderness green dragons & chaos altar, passive XP from early slayer, DT2'
      },
      {
        startExp: 737_627,
        rate: 122_900,
        description: 'Wilderness green dragons & chaos altar (108.3k prayer rate, incl. defence XP)'
      },
      {
        startExp: 13_034_431,
        rate: 137_800,
        description: 'Wilderness green dragons & chaos altar (108.3k prayer rate, incl. melee XP)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.COOKING,
    methods: [
      {
        startExp: 0,
        rate: 50_000,
        description: 'Quests, sardines, fly fishing'
      },
      {
        startExp: 13_363,
        rate: 211_200,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 41_171,
        rate: 240_800,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 101_333,
        rate: 270_400,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 273_742,
        rate: 300_100,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 737_627,
        rate: 329_700,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 1_986_068,
        rate: 359_300,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 5_346_332,
        rate: 385_900,
        description: 'Buying and cooking karambwan with an alt'
      },
      {
        startExp: 13_034_431,
        rate: 400_000,
        description: 'Buying and cooking karambwan with an alt'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.WOODCUTTING,
    methods: [
      {
        startExp: 0,
        rate: 29_000,
        description: "Monk's Friend and regular trees"
      },
      {
        startExp: 2_411,
        rate: 35_500,
        description: '2t oaks & burning logs'
      },
      {
        startExp: 13_363,
        rate: 44_300,
        description: '2t oaks & burning logs'
      },
      {
        startExp: 64_249,
        rate: 130_900,
        description: 'Wintertodt (1.5t teaks rate)'
      },
      {
        startExp: 302_288,
        rate: 75_000,
        description: 'Forestry for the log basket'
      },
      {
        startExp: 602_288,
        rate: 164_700,
        description: '1.5t teaks'
      },
      {
        startExp: 737_627,
        rate: 168_600,
        description: '1.5t teaks'
      },
      {
        startExp: 1_986_068,
        rate: 181_100,
        description: '1.5t teaks'
      },
      {
        startExp: 5_902_831,
        rate: 193_400,
        description: '1.5t teaks'
      },
      {
        startExp: 13_034_431,
        rate: 204_500,
        description: '1.5t teaks'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.FLETCHING,
    methods: [
      {
        startExp: 0,
        rate: 300_000,
        description: 'Headless arrows'
      },
      {
        startExp: 123_660,
        rate: 1_040_000,
        description: 'Broad arrows with Sepulchre, hunter, and artefacts'
      },
      {
        startExp: 13_034_431,
        rate: 1_300_000,
        description: 'Broad arrows with Sepulchre, hunter, and artefacts'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.FISHING,
    methods: [
      {
        startExp: 0,
        rate: 53_300,
        description: 'Quests and 3t fly fishing'
      },
      {
        startExp: 83_014,
        rate: 42_700,
        description: 'Eat-cut barbarian fishing'
      },
      {
        startExp: 224_466,
        rate: 70_900,
        description: 'Eat-cut barbarian fishing'
      },
      {
        startExp: 737_627,
        rate: 92_600,
        description: 'Eat-cut barbarian fishing'
      },
      {
        startExp: 2_183_968,
        rate: 153_100,
        description: 'Decant barblore (88.3k fishing rate & 400 caviar/h + potion noting time)'
      },
      {
        startExp: 5_346_332,
        rate: 169_600,
        description: 'Decant barblore (92.2k fishing rate & 432 caviar/h + potion noting time)'
      },
      {
        startExp: 13_034_431,
        rate: 183_000,
        description: 'Decant barblore (98.2k fishing rate & 456 caviar/h + potion noting time)'
      }
    ],
    bonuses: [
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.COOKING,
        startExp: 83_014,
        endExp: 13_034_431,
        end: true,
        ratio: 0.13
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.COOKING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.1355
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.AGILITY,
        startExp: 83_014,
        endExp: 224_466,
        end: false,
        ratio: 0.1
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.AGILITY,
        startExp: 224_466,
        endExp: 737_627,
        end: false,
        ratio: 0.093
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.AGILITY,
        startExp: 737_627,
        endExp: 2_183_968,
        end: false,
        ratio: 0.091
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.AGILITY,
        startExp: 2_183_968,
        endExp: 13_034_431,
        end: true,
        ratio: 0.091
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.AGILITY,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.0885
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.STRENGTH,
        startExp: 83_014,
        endExp: 224_466,
        end: false,
        ratio: 0.1
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.STRENGTH,
        startExp: 224_466,
        endExp: 737_627,
        end: false,
        ratio: 0.093
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.STRENGTH,
        startExp: 737_627,
        endExp: 2_183_968,
        end: false,
        ratio: 0.091
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.STRENGTH,
        startExp: 2_183_968,
        endExp: 13_034_431,
        end: true,
        ratio: 0.091
      },
      {
        originSkill: Skill.FISHING,
        bonusSkill: Skill.STRENGTH,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.0885
      }
    ]
  },
  {
    skill: Skill.FIREMAKING,
    methods: [
      {
        startExp: 0,
        rate: 7_200,
        description: 'Burning regular logs'
      },
      {
        startExp: 2_411,
        rate: 66_500,
        description: '2t oaks & burning oak logs'
      },
      {
        startExp: 50_339,
        rate: 80_500,
        description: '2t oaks & burning oak logs'
      },
      {
        startExp: 101_333,
        rate: 181_300,
        description: 'Dolo Wintertodt (164k firemaking rate, incl. passive & stored XP)'
      },
      {
        startExp: 273_742,
        rate: 215_000,
        description: 'Dolo Wintertodt (193k firemaking rate, incl. passive & stored XP)'
      },
      {
        startExp: 737_627,
        rate: 249_500,
        description: 'Dolo Wintertodt (223k firemaking rate, incl. passive & stored XP)'
      },
      {
        startExp: 1_986_068,
        rate: 287_600,
        description: 'Dolo Wintertodt (255k firemaking rate, incl. passive & stored XP)'
      },
      {
        startExp: 5_848_876,
        rate: 459_700,
        description: 'World-hop Wintertodt (360k firemaking rate, incl. passive & stored XP)'
      },
      {
        startExp: 13_034_431,
        rate: 431_300,
        description: 'World-hop Wintertodt (375k firemaking rate + stored XP)'
      }
    ],
    bonuses: [
      {
        originSkill: Skill.FIREMAKING,
        bonusSkill: Skill.WOODCUTTING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.049
      },
      {
        originSkill: Skill.FIREMAKING,
        bonusSkill: Skill.FLETCHING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.025
      }
    ]
  },
  {
    skill: Skill.CRAFTING,
    methods: [
      {
        startExp: 0,
        rate: 15_200,
        description: 'Quests, Port Phasmatys charters'
      },
      {
        startExp: 91_721,
        rate: 44_000,
        description: 'Port Phasmatys lantern lenses'
      },
      {
        startExp: 302_288,
        rate: 83_800,
        description: 'Charters (2-client hopping)'
      },
      {
        startExp: 3_972_294,
        rate: 103_800,
        description: 'Charters (2-client hopping)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.SMITHING,
    methods: [
      {
        startExp: 0,
        rate: 75_200,
        description: "Quests, Giants' Foundry"
      },
      {
        startExp: 273_742,
        rate: 315_000,
        description: 'Blast Furnace gold (2-client hopping)'
      },
      {
        startExp: 13_034_431,
        rate: 330_000,
        description: 'Blast Furnace gold (2-client hopping)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.MINING,
    methods: [
      {
        startExp: 0,
        rate: 8_000,
        description: 'Quests'
      },
      {
        startExp: 13_363,
        rate: 59_158,
        description: '3t iron'
      },
      {
        startExp: 61_512,
        rate: 85_500,
        description: '3t granite'
      },
      {
        startExp: 101_333,
        rate: 89_151,
        description: '3t granite'
      },
      {
        startExp: 302_288,
        rate: 97_923,
        description: '3t granite'
      },
      {
        startExp: 737_627,
        rate: 103_751,
        description: '3t granite'
      },
      {
        startExp: 1_986_068,
        rate: 109_143,
        description: '3t granite'
      },
      {
        startExp: 3_258_594,
        rate: 40_000,
        description: 'Motherlode Mine for the prospector kit'
      },
      {
        startExp: 3_808_594,
        rate: 113_287,
        description: '3t granite'
      },
      {
        startExp: 5_346_332,
        rate: 115_638,
        description: '3t granite'
      },
      {
        startExp: 13_034_431,
        rate: 125_000,
        description: '3t granite'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.HERBLORE,
    methods: [
      {
        startExp: 0,
        rate: 21_800,
        description: 'Quests, lamps, and master farmers'
      },
      {
        startExp: 737_627,
        rate: 38_169,
        description: 'Master farmers'
      },
      {
        startExp: 13_034_431,
        rate: 40_300,
        description: 'Master farmers, mole, and herbiboar + ToA time for fang'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.AGILITY,
    methods: [
      {
        startExp: 0,
        rate: 10_900,
        description: 'Bonus XP from fishing (Quests, low-level rooftops)'
      },
      {
        startExp: 123_660,
        rate: 44_900,
        description: 'Bonus XP from fishing (Sepulchre rate)'
      },
      {
        startExp: 273_742,
        rate: 40_000,
        description: "Seers' rooftop for the graceful outfit"
      },
      {
        startExp: 737_627,
        rate: 48_000,
        description: 'Pollnivneach rooftop for the graceful outfit'
      },
      {
        startExp: 1_282_357,
        rate: 65_700,
        description: 'Sepulchre'
      },
      {
        startExp: 2_421_087,
        rate: 73_700,
        description: 'Sepulchre looting (72.7k agility rate, incl. passive XP)'
      },
      {
        startExp: 6_517_253,
        rate: 91_800,
        description: 'Sepulchre looting (87.6k agility rate, incl. passive XP)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.THIEVING,
    methods: [
      {
        startExp: 0,
        rate: 26_000,
        description: 'Quests, fruit stalls'
      },
      {
        startExp: 61_512,
        rate: 80_000,
        description: 'Blackjacking'
      },
      {
        startExp: 166_636,
        rate: 130_000,
        description: 'Blackjacking'
      },
      {
        startExp: 449_428,
        rate: 200_000,
        description: 'Blackjacking'
      },
      {
        startExp: 1_210_421,
        rate: 225_000,
        description: 'Blackjacking'
      },
      {
        startExp: 3_258_594,
        rate: 245_000,
        description: 'Blackjacking, Pyramid Plunder for a sceptre'
      },
      {
        startExp: 7_944_614,
        rate: 339_866,
        description: 'Artefacts with glassblowing (215.3k thieving & 54.6k crafting rate, incl. supply time)'
      },
      {
        startExp: 13_034_431,
        rate: 355_354,
        description: 'Artefacts with glassblowing (224.9k thieving & 54.6k crafting rate, incl. supply time)'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.SLAYER,
    methods: [
      {
        startExp: 0,
        rate: 7_100,
        description:
          'Turael & Konar point boosting for 200 tasks (12.6k avg. slayer rate) + quest and gear time'
      },
      {
        startExp: 404_857,
        rate: 21_300,
        description: 'Konar slayer (19.5k slayer rate)'
      },
      {
        startExp: 668_051,
        rate: 16_400,
        description: 'Nieve slayer (33.2k slayer rate) + quest time and unlocks'
      },
      {
        startExp: 1_210_421,
        rate: 38_300,
        description: 'Nieve slayer (33.8k slayer rate)'
      },
      {
        startExp: 1_986_068,
        rate: 42_400,
        description: 'Duradel slayer (33.9k slayer rate)'
      },
      {
        startExp: 3_258_594,
        rate: 58_300,
        description: 'Duradel slayer (35.6k slayer rate)'
      },
      {
        startExp: 5_346_332,
        rate: 36_800,
        description: 'Duradel slayer (33.7k slayer rate) + PVM time'
      },
      {
        startExp: 7_195_629,
        rate: 63_000,
        description: 'Duradel slayer (38.6k slayer rate)'
      },
      {
        startExp: 13_034_431,
        rate: 71_800,
        description:
          'Duradel slayer (43.3k slayer rate) + passive & stored XP, PVM upgrades, and supply time)'
      }
    ],
    bonuses: [
      {
        originSkill: Skill.SLAYER,
        bonusSkill: Skill.RANGED,
        startExp: 0,
        endExp: 200_000_000,
        end: true,
        ratio: 0.025
      },
      {
        originSkill: Skill.SLAYER,
        bonusSkill: Skill.ATTACK,
        startExp: 0,
        endExp: 200_000_000,
        end: false,
        ratio: 1
      },
      {
        originSkill: Skill.SLAYER,
        bonusSkill: Skill.STRENGTH,
        startExp: 0,
        endExp: 200_000_000,
        end: false,
        ratio: 0.911
      },
      {
        originSkill: Skill.SLAYER,
        bonusSkill: Skill.DEFENCE,
        startExp: 0,
        endExp: 13_034_431,
        end: false,
        ratio: 0.605
      },
      {
        originSkill: Skill.SLAYER,
        bonusSkill: Skill.DEFENCE,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: false,
        ratio: 0.663
      }
    ]
  },
  {
    skill: Skill.FARMING,
    methods: [
      {
        startExp: 0,
        rate: 115_000,
        description: 'Quests, low-level trees'
      },
      {
        startExp: 313_818,
        rate: 65_000,
        description: 'Tithe Farm rewards'
      },
      {
        startExp: 496_254,
        rate: 800_000,
        description: 'Tree runs from mole seeds'
      },
      {
        startExp: 13_034_431,
        rate: 1_090_000,
        description: 'Tree runs from mole seeds'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.RUNECRAFTING,
    methods: [
      {
        startExp: 0,
        rate: 13_100,
        description: 'Quests'
      },
      {
        startExp: 37_224,
        rate: 36_100,
        description: 'GotR small teams / dolos (35.2k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 101_333,
        rate: 53_000,
        description: 'GotR small teams / dolos (43.2k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 449_428,
        rate: 58_300,
        description: 'GotR small teams / dolos (47.5k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 1_210_421,
        rate: 59_900,
        description: 'GotR small teams / solos (56.1k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 1_475_581,
        rate: 60_600,
        description: 'GotR small teams / solos (56.7k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 3_258_594,
        rate: 74_400,
        description: 'GotR small teams / solos (69.5k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 8_771_558,
        rate: 78_600,
        description: 'GotR small teams / solos (73.4k runecraft rate + passive XP & looting time)'
      },
      {
        startExp: 13_034_431,
        rate: 75_128,
        description: 'GotR small teams / solos (77.0k runecraft rate + looting time)'
      }
    ],
    bonuses: [
      {
        originSkill: Skill.RUNECRAFTING,
        bonusSkill: Skill.CRAFTING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.095
      },
      {
        originSkill: Skill.RUNECRAFTING,
        bonusSkill: Skill.MINING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.0364
      }
    ]
  },
  {
    skill: Skill.HUNTER,
    methods: [
      {
        startExp: 0,
        rate: 72_200,
        description: 'Low-level birdhouses'
      },
      {
        startExp: 91_721,
        rate: 103_800,
        description: 'Mahogany birdhouses, red salamanders'
      },
      {
        startExp: 737_627,
        rate: 104_600,
        description: 'Red chinchompas'
      },
      {
        startExp: 1_986_068,
        rate: 158_300,
        description: 'Red chinchompas'
      },
      {
        startExp: 5_346_332,
        rate: 183_300,
        description: 'Red chinchompas'
      },
      {
        startExp: 13_034_431,
        rate: 200_000,
        description: 'Red chinchompas, herbiboar'
      }
    ],
    bonuses: []
  },
  {
    skill: Skill.CONSTRUCTION,
    methods: [
      {
        startExp: 0,
        rate: 3_800,
        description: 'Quests, low-level furniture'
      },
      {
        startExp: 4_470,
        rate: 0,
        description: 'Dolo Wintertodt'
      },
      {
        startExp: 223_678,
        rate: 128_100,
        description: '1.5t teaks & Mahogany Homes (107k base rate, no plank sack)'
      },
      {
        startExp: 508_945,
        rate: 138_400,
        description: '1.5t teaks & Mahogany Homes (114k base rate)'
      },
      {
        startExp: 1_336_443,
        rate: 144_600,
        description: '1.5t teaks & Mahogany Homes (119k base rate)'
      },
      {
        startExp: 1_475_581,
        rate: 219_944,
        description: '1.5t teaks & mythical capes (125k base rate)'
      },
      {
        startExp: 13_034_431,
        rate: 125_000,
        description: '1.5t teaks & mythical capes'
      }
    ],
    bonuses: [
      {
        originSkill: Skill.CONSTRUCTION,
        bonusSkill: Skill.WOODCUTTING,
        startExp: 13_034_431,
        endExp: 200_000_000,
        end: true,
        ratio: 0.706
      }
    ]
  }
];
