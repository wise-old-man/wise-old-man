...
export const DispatchCompetitionStartedDiscordEventJobHandler: JobHandler<Payload> = {
  options: {
    backoff: {
      type: 'exponential',
      delay: 30_000
  ...
  async execute(payload) {
    ...
    const dispatchResult = await dispatchDiscordBotEvent(DiscordBotEventType.COMPETITION_STARTED, {
