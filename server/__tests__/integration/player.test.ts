...
import { BOSSES, Metrics, PlayerType } from '@wise-old-man/utils';
...
describe('Player API', () => {
  describe('1. Tracking', () => {
    ...
    it('should track player (1 def)', async () => {
      ...
      const responseDef1 = await api.post(`/api/players/track`).send({ username: '1def' });

      expect(responseDef1.status).toBe(201);
      expect(responseDef1.body.build).toBe('1def');
    });
    ...
    it('should track player (10hp)', async () => {
      ...
      const response10HP = await api.post(`/api/players/track`).send({ username: '10hp' });

      expect(response10HP.status).toBe(201);
      expect(response10HP.body.build).toBe('10hp');
    });
