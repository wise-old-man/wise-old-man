...
describe('Player API', () => {
  ...
  describe('10. Archiving', () => {
    it("shouldn't auto-archive, send discord flagged report instead (excessive gains)", async () => {
      ...
      const previousSnapshot = await snapshotUtils.parseHiscoresSnapshot(
        player.id,
        globalData.hiscoresRawData
      );
      ...
      const rejectedSnapshot = await snapshotUtils.parseHiscoresSnapshot(player.id, modifiedRawData);
    ...
    it("shouldn't auto-archive, send discord flagged report instead (negative gains, possible rollback)", async () => {
      ...
      const rejectedSnapshot = await snapshotUtils.parseHiscoresSnapshot(player.id, modifiedRejectedRawData);
    ...
    it("should auto-archive, and not send discord flagged report (negative gains, excessive gains reversed, can't be a rollback)", async () => {
      ...
      const previousSnapshot = await snapshotUtils.parseHiscoresSnapshot(player.id, modifiedPreviousRawData);
      ...
      const rejectedSnapshot = await snapshotUtils.parseHiscoresSnapshot(
        player.id,
        globalData.hiscoresRawData
      );
