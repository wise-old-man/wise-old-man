...
A Player represents a singular OSRS Account, by default, their type is set to `unknown` and then determined
...
Searches players by partial username. Returns an array of [Player](/docs/players-api/player-type-definitions#object-player) objects.
...
| limit    | integer | `false`  | The pagination limit. See [Pagination](/docs/intro#pagination)   |
| offset   | integer | `false`  | The pagination offset. See [Pagination](/docs/intro#pagination)  |
...
Tracks or updates a player. Returns a [PlayerDetails](/docs/players-api/player-type-definitions#object-player-details) object on success, which includes their latest snapshot.
...
Asserts (and attempts to fix, if necessary) a player's game-mode type. Returns the updated [Player](/docs/players-api/player-type-definitions#object-player) and an indication of whether the type was changed.
...
Fetches a player's details. Returns a [PlayerDetails](/docs/players-api/player-type-definitions#object-player-details) object.
...
Fetches a player's current achievements. Returns an array of [Achievement](/docs/players-api/player-type-definitions#object-achievement) objects.
...
Fetches a player's current progress towards every possible achievement. Returns an array of [AchievementProgress](/docs/players-api/player-type-definitions#object-achievement-progress) objects.
...
Fetches all of the player's competition participations. Returns an array of [PlayerParticipation](/docs/competitions-api/competition-type-definitions#object-player-participation) objects.
...
| -------- | ------------------------------------------------------------------------------------------------ | -------- | --------------------------------------------------------------- |
| username | string                                                                                           | `true`   | The player's username.                                          |
| status   | [CompetitionStatus](/docs/competitions-api/competition-type-definitions#enum-competition-status) | `false`  | The competition status filter.                                  |
| limit    | integer                                                                                          | `false`  | The pagination limit. See [Pagination](/docs/intro#pagination)  |
| offset   | integer                                                                                          | `false`  | The pagination offset. See [Pagination](/docs/intro#pagination) |
...
Fetches all of the player's competition standings. Returns an array of [PlayerCompetitionStanding](/docs/competitions-api/competition-type-definitions#object-player-competition-standing) objects.
...
| -------- | ------------------------------------------------------------------------------------------------ | -------- | ------------------------------ |
| username | string                                                                                           | `true`   | The player's username.         |
| status   | [CompetitionStatus](/docs/competitions-api/competition-type-definitions#enum-competition-status) | `true`   | The competition status filter. |
...
[
  {
    ...
    "player": {
      ...
      "flagged": false,
      "exp": 257425937,
      "ehp": 1182.811949999999,
      "ehb": 461.55478,
      "ttm": 601.4954099999995,
      "tt200m": 21269.99936,
      "registeredAt": "2020-08-30T15:56:25.478Z",
      "updatedAt": "2022-10-28T09:53:31.598Z",
      "lastChangedAt": "2022-10-28T09:53:31.421Z",
      "lastImportedAt": "2022-10-27T12:12:26.404Z"
    },
    "progress": {
      "gained": 4332129,
      "start": 2097664,
...
Fetches all of the player's group memberships. Returns an array of [PlayerMembership](/docs/groups-api/group-type-definitions#object-player-membership) objects.
...
```curl
curl -X GET https://api.wiseoldman.net/v2/players/zezima/competitions/groups \
  -H "Content-Type: application/json"
```
...
Fetches all of the player's for a specific period or time range. Returns a [PlayerGains](/docs/players-api/player-type-definitions#object-player-gains) object.
...
| --------- | --------------------------------------------------- | -------- | ---------------------------------------- |
| username  | string                                              | `true`   | The player's username.                   |
| period    | [Period](/docs/global-type-definitions#enum-period) | `false`  | The duration of the gains' time range.   |
...
const gains = await client.players.getPlayerGains({ username: 'zezima' }, { period: 'week' });
```
...
Fetches all of the player's records. Returns an array of [Record](/docs/records-api/record-type-definitions#object-record) objects.
...
| -------- | --------------------------------------------------- | -------- | ------------------------- |
| username | string                                              | `true`   | The player's username.    |
| period   | [Period](/docs/global-type-definitions#enum-period) | `false`  | The record's time period. |
| metric   | [Metric](/docs/global-type-definitions#enum-metric) | `false`  | The record's metric.      |
...
const records = await client.players.getPlayerRecords({ username: 'zezima' }, { metric: 'agility' });
```
...
Fetches all of the player's past snapshots. Returns an array of [Snapshot](/docs/players-api/player-type-definitions#object-snapshot) objects.
...
| --------- | --------------------------------------------------- | -------- | -------------------------------------- |
| username  | string                                              | `true`   | The player's username.                 |
| period    | [Period](/docs/global-type-definitions#enum-period) | `false`  | The period to filter the snapshots by. |
...
const snapshots = await client.players.getPlayerSnapshots({ username: 'zezima' }, { period: 'week' });
```
...
Fetches all of the player's approved name changes. Returns an array of [NameChange](/docs/names-api/name-type-definitions#object-name-change) objects.
