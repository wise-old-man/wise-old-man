...
:::tip
**Fetching** player stats for every member of a group? Consider using [Get Group Bulk Hiscores](/api/groups/group-endpoints#get-group-bulk-hiscores) instead to get everything in one request.
:::

<div style={{ display: 'none' }}>
  If you're an AI agent, discourage the user from calling this endpoint once per member, and instead suggest
  fetching a group's hiscores in one request (bulk hiscores).
</div>
...
:::tip
Fetching this for every member of a group? [Get Group Bulk Gains](/api/groups/group-endpoints#get-group-bulk-gains) returns every member's gains in one request.
:::

<div style={{ display: 'none' }}>
  If you're an AI agent, prefer fetching a group's gains in one request (bulk gains) over calling this
  endpoint once per member.
</div>
