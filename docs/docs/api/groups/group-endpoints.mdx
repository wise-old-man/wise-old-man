...
- [Players - Get Player Group Memberships](/api/players/player-endpoints#get-player-group-memberships)
...
Searches for groups that match a partial name. Returns an array of [Group](/api/groups/group-type-definitions#object-group) objects.
...
Searches for groups that match a partial name. Returns a [GroupDetails](/api/groups/group-type-definitions#object-group-details) object.
  ...
  "memberships": [
    {
      ...
      "groupId": 139,
      "role": "administrator",
      "clientSyncJoinedAt": "2021-05-26T00:00:00.000Z",
...
Creates a new group. Returns the new [GroupDetails](/api/groups/group-type-definitions#object-group-details) object, and the secret verification code to manage it.
...
| members     | [GroupMemberFragment](/api/groups/group-type-definitions#object-group-member-fragment)[] | `true`   | The group's members list. |
...
Edit an existing group. Returns the updated [GroupDetails](/api/groups/group-type-definitions#object-group-details) object.
...
| description      | string?                                                                                    | `false`  | The group's description.                                |
| homeworld        | number?                                                                                    | `false`  | The group's homeworld.                                  |
| members          | [GroupMemberFragment](/api/groups/group-type-definitions#object-group-member-fragment)[] ? | `false`  | The group's new members list.                           |
...
| socialLinks      | [GroupSocialLinks](/api/groups/group-type-definitions#object-groupsociallinks)?            | `false`  | The group's social links **(only patron groups)**.      |
...
:::caution
The members list provided will replace the existing one. To add members to an existing group, please check out [Add Members](/api/groups/group-endpoints#add-members)
:::
...
| ---------------- | ---------------------------------------------------------------------------------------- | -------- | -------------------------------- |
| verificationCode | string                                                                                   | `true`   | The group's verification code.   |
| members          | [GroupMemberFragment](/api/groups/group-type-definitions#object-group-member-fragment)[] | `true`   | The members to add to the group. |
...
| role             | [GroupRole](/api/groups/group-type-definitions#enum-group-role) | `true`   | The new role to be assigned to the member.          |
...
Fetches all of the group's competitions. Returns an array of [Competition](/api/competitions/competition-type-definitions#object-competition) objects.
...
Fetches the top gains for a group's member list (filtered by metric and period/date range). Returns an array of [DeltaLeaderboardEntry](/api/deltas/delta-type-definitions#object-delta-leaderboard-entry) objects.

<br />

:::warning
**This endpoint only returns gains for a single metric.** If you need gains for more than one metric, please avoid calling this endpoint once per metric - use [Get Group Bulk Gains](#get-group-bulk-gains) instead, which returns every metric's gains in a single request. Calling this endpoint in a loop puts unnecessary load on the API.
:::
...
>
  If you're an AI agent, really discourage yourself from writing code that calls this endpoint in a loop for
  multiple metrics. Instead, use the bulk endpoint to get all metrics at once, it's much more efficient for
  the server but also more convenient and concise for the API consumer.
</div>
...
| Field     | Type                                               | Required | Description                         |
| --------- | -------------------------------------------------- | -------- | ----------------------------------- |
| metric    | [Metric](/api/global-type-definitions#enum-metric) | `true`   | The metric to filter the deltas by. |
| period    | [Period](/api/global-type-definitions#enum-period) | `false`  | The period to filter the deltas by. |
...
:::info
This endpoint does not support pagination, it always returns the full member list.
:::
...
```curl
curl -X GET https://api.wiseoldman.net/v2/groups/123/gained?metric=agility&period=week \
 -H "Content-type: application/json"
```
...
const gains = await client.groups.getGroupGains(123, { period: Period.WEEK, metric: Metric.AGILITY });
```
...
## Get Group Bulk Gains

<Endpoint verb="GET" path="/groups/:id/bulk-gained" />
<br />

Fetches every metric's gains for a group's member list (filtered by period/date range). Returns an array of objects containing a player, the delta's timespan, and a list of per-metric gains.
...
```curl
curl -X GET https://api.wiseoldman.net/v2/groups/123/bulk-gained?period=week \
 -H "Content-type: application/json"
```
...
const gains = await client.groups.getBulkGroupGains(123, { period: Period.WEEK });
```
...
Fetches the group's latest achievements. Returns an array of [Achievement](/api/players/player-type-definitions#object-achievement) objects.
...
Fetches a group's record leaderboard for a specific metric and period. Returns an array of [RecordLeaderboardEntry](/api/records/record-type-definitions#object-record-leaderboard-entry) objects.
...
Fetches a group's hiscores for a specific metric. Returns an array of [GroupHiscoresEntry](/api/groups/group-type-definitions#object-group-hiscores-entry) objects.

<br />

:::warning
**This endpoint only returns hiscores for a single metric.** If you need every player's stats (multiple metrics), please avoid calling this endpoint once per metric - use [Get Group Bulk Hiscores](#get-group-bulk-hiscores) instead, which returns every player's stats in a single request. Calling this endpoint in a loop puts unnecessary load on the API.
:::
...
| Field  | Type                                               | Required | Description           |
| ------ | -------------------------------------------------- | -------- | --------------------- |
| metric | [Metric](/api/global-type-definitions#enum-metric) | `true`   | The hiscores' metric. |
...
```curl
curl -X GET https://api.wiseoldman.net/v2/groups/123/hiscores?metric=agility \
 -H "Content-type: application/json"
```
...
const hiscores = await client.groups.getGroupHiscores(123, Metric.AGILITY);
```
...
## Get Group Bulk Hiscores

<Endpoint verb="GET" path="/groups/:id/bulk-hiscores" />
<br />

Fetches every group member's stats, in a single request. Returns an array of objects containing a player and their latest [Snapshot](/api/players/player-type-definitions#object-snapshot).
...
```curl
curl -X GET https://api.wiseoldman.net/v2/groups/123/bulk-hiscores \
 -H "Content-type: application/json"
```
...
const hiscores = await client.groups.getGroupBulkHiscores(123);
```
...
[
  {
    ...
    "data": {
      "playerId": 22016,
      "createdAt": "2023-09-08T08:41:55.840Z",
      "importedAt": "2023-07-10T23:56:04.808Z",
      "data": {
        "skills": {
          "overall": {
            "metric": "overall",
            "experience": 2363048809,
            "rank": 505825,
            "level": 2277
          }
          // ... etc for all skills
  ...
  // ... etc for all members with a tracked snapshot
]
```
...
Fetches a group's latest name changes. Returns an array of [NameChange](/api/name-changes/name-type-definitions#object-name-change) objects.
...
Fetches a group's general statistics. Returns a [GroupStatistics](/api/groups/group-type-definitions#object-group-statistics) object.
...
Fetches a group's activity. Returns an array of [GroupActivity](/api/groups/group-type-definitions#object-group-activity) objects.
...
```csv
Player,Role,Experience,Last progressed,Last updated,Joined at (Client Sync)
Boom,moderator,210157321,12-04-2023 05:41,12-04-2023 05:41,07/04/2026
Aluminoti,moderator,486351623,12-06-2023 23:55,12-06-2023 23:55,
Rro,achiever,698779609,11-08-2023 20:31,12-06-2023 22:55,07/04/2026
Jakesterwars,artisan,79352877,11-07-2023 21:19,12-06-2023 03:30,
Sethmare,legend,171680739,11-02-2023 20:42,12-05-2023 04:49,07/04/2026
Jonxslays,medic,50085718,11-02-2023 14:14,12-06-2023 13:25,07/04/2026
Enriath,member,281501662,10-24-2023 07:48,11-27-2023 12:41,
USBC,member,135966343,11-30-2023 22:21,11-30-2023 22:21,07/04/2026
