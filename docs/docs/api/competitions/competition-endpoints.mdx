...
Fetches all the values (exp, kc, etc) in chronological order within the bounds of the competition, for the top 5 participants. Returns an array of [ParticipantHistory](/api/competitions/competition-type-definitions#object-participant-history-result) objects.
