...
export function SailingTimeseriesChart(props: Props) {
  const [mode, setMode] = useState<"sum" | "avg">("sum");

  const datasets = [
    {
      name: mode === "sum" ? "Sum Sailing Exp." : "Avg. Sailing Exp.",
    ...
    <div className="flex flex-col gap-y-5 rounded-lg border border-gray-500 px-5 py-4">
      <div className="flex flex-row justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-200">Game-wide {mode} exp. over time</p>
        </div>
