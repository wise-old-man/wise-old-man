...
function RemoveGroupMembersModal({ groupId, members, onConfirm, onClose }) {
    ...
    <div className="remove-group-members">
      <div className="remove-group-members__modal">
        ...
        <b className="modal-title">
          Are you sure you want to remove the following members from this group?
        </b>
        <p>{members.map(m => m.displayName).join(', ')}</p>
        <Button text="Remove group members" onClick={onConfirm} />
      </div>
    </div>
  );
}
